{{-- resources/views/livewire/lista-orcamento-concluidos.blade.php --}}
{{--
    ATENÇÃO: a coluna PDF agora aponta para o comprovante do PAGAMENTO
    ($pagamento->pdf_path), não para o PDF do orçamento ($o->pdf_path).
    O link abre inline no navegador via rota pagamentos.comprovante-pdf.
--}}
    ...
    <div class="flex items-end gap-4 px-6 py-4">
        <div class="flex items-end gap-4 flex-wrap">
            ...
            <div class="flex flex-col flex-[2] min-w-[150px]">
                <label for="loadingDay" class="text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">Dia Carreg.</label>
            ...
            <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                @forelse ($orcamentos as $o)
                    {{--
                        ...
                        ->first() aqui é só para pegar o item da coleção em memória,
                        sem nova query.
                    --}}
                    @php $pagamento = $o->pagamentos->first(); @endphp
                    <tr class="hover:bg-zinc-100 dark:hover:bg-zinc-700 transition">

                        {{-- Obra --}}
                        <td class="px-6 py-4 text-zinc-800 dark:text-zinc-200">
                            <a href="{{ route('orcamentos.show', $o) }}" class="hover:underline font-medium">
                                {{ $o->obra }}
                            </a>
                        </td>
                        ...
                        <td class="px-6 py-4">
                            @if($pagamento && $pagamento->temPdf())
                                <a href="{{ route('pagamentos.comprovante-pdf', $pagamento) }}"
                                   target="_blank"
                                   title="Abrir comprovante do pagamento #{{ $pagamento->id }}"
                            ...
                            @elseif($pagamento && !$pagamento->temPdf())
                                {{-- Pagamento existe mas PDF falhou --}}
