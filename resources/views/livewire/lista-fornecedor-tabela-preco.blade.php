...
<div class="bg-white dark:bg-zinc-900 shadow rounded-2xl border border-zinc-200 dark:border-zinc-700">
    ...
    <div class="flex items-center justify-between p-6 border-b border-zinc-200 dark:border-zinc-700">
        <h2 class="text-2xl font-bold text-zinc-900 dark:text-zinc-50">
            Listagem de preços do Fornecedor 
        </h2>
                ...
                @forelse($produtos as $p)
                    <tr class="hover:bg-zinc-100 dark:hover:bg-zinc-700 transition">
                        <td class="px-6 py-4 text-zinc-800 dark:text-zinc-200"><a
                                href="/produtos/{{ $p->id }}">{{ $p->nome }}</a></td>
                        <td class="px-6 py-4 text-zinc-800 dark:text-zinc-200"><a
                                href="/produtos/{{ $p->id }}">{{ $p->preco_custo }}</a></td>
                        <td class="px-6 py-4 text-zinc-800 dark:text-zinc-200"><a
                                href="/produtos/{{ $p->id }}">{{ $p->preco_venda }}</a></td>
                        <td class="px-6 py-4 font-mono text-zinc-800 dark:text-zinc-200">
                            {{ $p->updated_at->format('d/m/Y H:i') }}</td>
                    </tr>
                @empty
                    <tr>
