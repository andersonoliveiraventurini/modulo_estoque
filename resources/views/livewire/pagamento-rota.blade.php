            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="space-y-6">
                    ...
                    <div class="ring-1 ring-gray-200 dark:ring-gray-700 rounded-lg overflow-hidden">
                        ...
                        <div class="bg-white dark:bg-zinc-900 p-4 space-y-4">
                            <div class="flex items-center justify-between">
                                <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Tipo de Documento</span>
                                <div class="flex bg-gray-100 dark:bg-gray-800 p-1 rounded-lg">
                                    <button type="button" 
                                        wire:click="$set('precisaNotaFiscal', false)"
                                        class="px-3 py-1.5 text-xs font-bold rounded-md transition-all {{ !$precisaNotaFiscal ? 'bg-white dark:bg-gray-700 shadow-sm text-indigo-600 dark:text-indigo-400' : 'text-gray-500 hover:text-gray-700' }}">
                                        Cupom Fiscal
                                    </button>
                                    <button type="button" 
                                        wire:click="$set('precisaNotaFiscal', true)"
                                        class="px-3 py-1.5 text-xs font-bold rounded-md transition-all {{ $precisaNotaFiscal ? 'bg-white dark:bg-gray-700 shadow-sm text-indigo-600 dark:text-indigo-400' : 'text-gray-500 hover:text-gray-700' }}">
                                        Nota Fiscal (NFe)
                                    </button>
                                </div>
                            ...
                            @if($precisaNotaFiscal)
                                <div class="space-y-3 pt-3 border-t border-gray-100 dark:border-gray-800 animate-fade-in">
                                    <div class="flex items-center justify-between">
                                        <label class="text-xs font-medium text-gray-600 dark:text-gray-400">Dados da Nota</label>
                                        <label class="relative inline-flex items-center cursor-pointer">
                                            <input type="checkbox" wire:model.live="notaOutroCnpjCpf" class="sr-only peer">
                                            <div class="w-9 h-5 bg-gray-200 peer-focus:outline-none rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all dark:border-gray-600 peer-checked:bg-indigo-600"></div>
                                            <span class="ml-2 text-[10px] font-medium text-gray-700 dark:text-gray-300">Outro CNPJ/CPF</span>
                                        </label>
                                    </div>
                                    ...
                                    @if($notaOutroCnpjCpf)
                                        <div class="animate-fade-in-down">
                                            <input type="text" wire:model="cnpjCpfNota" 
                                                class="w-full border border-gray-300 dark:border-gray-600 rounded-md px-3 py-2 text-sm bg-white dark:bg-zinc-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-indigo-500"
                                                placeholder="Informe o CNPJ ou CPF para a nota...">
                                        </div>
                                    @else
                                        <div class="p-2 bg-gray-50 dark:bg-gray-800/50 rounded-md border border-gray-100 dark:border-gray-800">
                                            <p class="text-[10px] text-gray-500 uppercase font-bold tracking-wider">Usar dados do cliente:</p>
                                            <p class="text-xs font-medium text-gray-700 dark:text-gray-300">{{ $orcamento->cliente->nome }}</p>
                                            <p class="text-xs text-gray-500">{{ $orcamento->cliente->cnpj_cpf }}</p>
                                        </div>
                                    @endif
                                </div>
                            @endif
