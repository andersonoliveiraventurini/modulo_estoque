<div class="mx-auto max-w-7xl px-4 py-6">
    ...
    @if (session('success'))
        <div class="mb-3 rounded border border-emerald-300 dark:border-emerald-700 bg-emerald-50 dark:bg-emerald-900/30 px-3 py-2 text-emerald-800 dark:text-emerald-200">
            {{ session('success') }}
        </div>
    @endif
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-800">
                        <tr>
                            ...
                            <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Separada</th>
                            <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Conferida</th>
                                ...
                                <td class="px-3 py-2">
                                    <div class="flex flex-col gap-2">
                                        ...
                                        <div class="flex items-center gap-2">
                                            <button
                                                wire:click="salvarItem({{ $it->id }})"
                                                class="px-3 py-1 rounded bg-indigo-600 hover:bg-indigo-700 text-white text-sm">
                                                Salvar
                                            </button>
                                            @if($it->status === 'divergente')
                                                <span class="inline-flex items-center px-2 py-1 rounded text-xs bg-rose-100 text-rose-800 dark:bg-rose-900/40 dark:text-rose-200">
                                                    Divergente ({{ rtrim(rtrim(number_format($it->divergencia,3,',','.'),'0'),',') }})
                                                </span>
                                            @else
                                        ...
                                        @if($it->conferido_por_id)
                                            <div class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                                Conferido por #{{ $it->conferido_por_id }} em {{ optional($it->conferido_em)->format('d/m/Y H:i') }}
                                            </div>
                                        @endif
                                    </div>
                                </td>
