<div class="p-6 bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-neutral-200 dark:border-neutral-700">
    <div class="mb-6">
        <h2 class="text-xl font-bold text-neutral-900 dark:text-white">Solicitar Devolução {{ $pedidoId ? "- Pedido #$pedidoId" : "" }}</h2>
        <p class="text-sm text-neutral-500">
            @if($pedidoId)
                Selecione os itens e quantidades que deseja devolver.
            @else
                Por favor, acesse esta página através de um pedido específico para solicitar a devolução.
            @endif
        </p>
    </div>
    ...
    @else
        <div class="py-12 flex flex-col items-center justify-center text-neutral-500">
            <flux:icon icon="arrow-path" class="w-12 h-12 mb-4 opacity-20" />
            <p>Selecione um pedido no <a href="{{ route('historico.financeiro') }}" class="text-primary-600 hover:underline">Histórico Financeiro</a> para iniciar uma devolução.</p>
        </div>
    @endif
</div>
