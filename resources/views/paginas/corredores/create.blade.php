<x-layouts.app title="Novo Corredor">
    <div class="flex w-full flex-1 flex-col gap-4 rounded-xl">
        <div class="relative h-full flex-1 overflow-hidden rounded-xl border border-neutral-200 dark:border-neutral-700">
            <div class="bg-white p-6 shadow rounded-2xl border-e border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900 max-w-2xl">
                ...
                <form action="{{ route('corredores.store') }}" method="POST" class="space-y-6">
                    @csrf
                    <div>
                        <x-select name="armazem_id" label="Armazém">
                            <option value="">Selecione o Armazém</option>
                            @foreach($armazens as $armazem)
                                <option value="{{ $armazem->id }}" {{ old('armazem_id') == $armazem->id ? 'selected' : '' }}>{{ $armazem->nome }}</option>
                            @endforeach
                        </x-select>
                    ...
                    <div>
                        <x-input name="nome" label="Nome / Identificação do Corredor" placeholder="Ex: A, 01, Corredor Frio" value="{{ old('nome') }}" />
                        @error('nome') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <x-input name="descricao" label="Descrição" placeholder="Detalhes adicionais do corredor" value="{{ old('descricao') }}" />
