<x-layouts.app title="Editar Pedido de Compra #{{ $pedidoCompra->id }}">
            ...
            <div class="bg-white p-6 shadow rounded-2xl bg-zinc-50 dark:bg-zinc-900">
                ...
                <div class="flex justify-between items-center mb-6">
                    <h2 class="text-xl font-semibold flex items-center gap-2 text-gray-900 dark:text-gray-100">
                        ...
                        Editar Pedido de Compra #{{ $pedidoCompra->id }}
                    </h2>
                ...
                <form action="{{ route('pedido_compras.update', $pedidoCompra->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <x-input name="numero_pedido" label="Nº do Pedido / Referência" placeholder="Ex: PO-2024-001" value="{{ old('numero_pedido', $pedidoCompra->numero_pedido) }}" />
                            <x-input name="data_pedido" label="Data do Pedido *" type="date" value="{{ old('data_pedido', $pedidoCompra->data_pedido->format('Y-m-d')) }}" />
                            <x-input name="previsao_entrega" label="Previsão de Entrega" type="date" value="{{ old('previsao_entrega', optional($pedidoCompra->previsao_entrega)?->format('Y-m-d')) }}" />

                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Status</label>
                                <select name="status" class="mt-1 block w-full border-gray-300 dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-300 focus:border-indigo-500 rounded-md shadow-sm">
                                    @foreach(['aguardando' => 'Aguardando', 'parcialmente_recebido' => 'Parcialmente Recebido', 'recebido' => 'Recebido', 'cancelado' => 'Cancelado'] as $val => $label)
                                        <option value="{{ $val }}" {{ (old('status') ?? $pedidoCompra->status) == $val ? 'selected' : '' }}>{{ $label }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <select name="condicao_pagamento_id" class="mt-1 block w-full border-gray-300 dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-300 focus:border-indigo-500 rounded-md shadow-sm">
                                    ...
                                    @foreach($condicoes as $c)
                                        <option value="{{ $c->id }}" {{ (old('condicao_pagamento_id') ?? $pedidoCompra->condicao_pagamento_id) == $c->id ? 'selected' : '' }}>{{ $c->nome }}</option>
                                    @endforeach
                                </select>
                            </div>

                            <x-input name="forma_pagamento_descricao" label="Detalhes Pag. / Parcelas" placeholder="Ex: 30/60/90 dias" value="{{ old('forma_pagamento_descricao', $pedidoCompra->forma_pagamento_descricao) }}" />
                            ...
                            <div>
                                ...
                                @if($pedidoCompra->arquivo_pedido)
                                    <div class="text-xs text-indigo-600 dark:text-indigo-400 mb-1">
                                        Atual: <a href="{{ Storage::url($pedidoCompra->arquivo_pedido) }}" target="_blank" class="underline">Ver arquivo</a>
                                    </div>
                                @endif
                                ...
                                @if($pedidoCompra->arquivo_pedido)
                                    <p class="mt-1 text-xs text-gray-500">Já existe um arquivo enviado.</p>
                                @endif
                            </div>

                            <div class="md:col-span-3">
                                <x-input name="observacao" label="Observações" placeholder="Informações adicionais..." value="{{ old('observacao', $pedidoCompra->observacao) }}" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4 pt-4 border-t border-gray-200 dark:border-neutral-700">
                        ...
                        <div class="text-right mt-4 p-4 bg-gray-50 dark:bg-neutral-800 rounded-lg">
                            ...
                            <span class="text-xl font-bold text-gray-900 dark:text-gray-100" id="total-geral">R$ {{ number_format($pedidoCompra->valor_total ?? 0, 2, ',', '.') }}</span>
                        </div>
                    </div>

                    <div class="flex gap-4 pt-4">
                        <x-button type="submit" variant="primary">Salvar Alterações</x-button>
                        <a href="{{ route('pedido_compras.index') }}" class="px-4 py-2 border border-gray-300 dark:border-neutral-700 rounded-md font-semibold text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-neutral-800">Cancelar</a>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div id="modal-quantidade" class="fixed inset-0 bg-black/50 flex items-center justify-center hidden z-50">
        <div class="bg-white dark:bg-zinc-900 rounded-lg p-6 w-96 shadow-xl relative">
            ...
            <h3 class="text-lg font-semibold mb-2 text-gray-900 dark:text-white">Adicionar/Editar Item</h3>
    ...
    <script>
        ...
        function confirmarAdicao() {
            ...
            if (existe !== -1) {
                ...
                produtosNoPedido[existe].valor_unitario = preco;
            } else {
