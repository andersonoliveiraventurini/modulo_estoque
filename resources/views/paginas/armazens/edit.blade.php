<x-layouts.app title="Editar Armazém">
        ...
        <div class="relative h-full flex-1 overflow-hidden rounded-xl border border-neutral-200 dark:border-neutral-700">
            <div class="bg-white p-6 shadow rounded-2xl border-e border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900 max-w-2xl">
                <h2 class="text-xl font-semibold flex items-center gap-2 mb-6 text-gray-900 dark:text-gray-100">
                    <x-heroicon-o-pencil-square class="w-6 h-6 text-indigo-500" />
                    Editar Armazém #{{ $armazem->id }}
                </h2>

                <form action="{{ route('armazens.update', $armazem->id) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('PUT')
                    <div>
                        <x-input name="nome" label="Nome do Armazém *" placeholder="Ex: Galpão A, Principal, Frio" value="{{ old('nome', $armazem->nome) }}" />
                        @error('nome') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <x-input name="localizacao" label="Localização / Endereço" placeholder="Ex: Rua X, Setor Y" value="{{ old('localizacao', $armazem->localizacao) }}" />
