...
@php
    ...
    // Flags de conteúdo
    $temProdutos   = $orcamento->itens->whereNotNull('produto_id')->count() > 0;
    $temEncomenda  = ($orcamento->encomenda ?? false)
                     && isset($grupoCotacao)
                     && $grupoCotacao
                     && $grupoCotacao->itens->count() > 0;
@endphp
...
{{-- ===========================
     ITENS DE ENCOMENDA (COTAÇÃO)
=========================== --}}
@if ($temEncomenda)
    <h3 style="margin-top: {{ $temProdutos ? '15px' : '0' }}">
        Itens da Encomenda
        <span style="font-size:11px; font-weight:normal; color:#666;">(Cotação #{{ $grupoCotacao->id }})</span>
    </h3>
    <table>
        <thead>
        <tr>
