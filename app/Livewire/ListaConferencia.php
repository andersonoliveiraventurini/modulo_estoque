...
class ListaConferencia extends Component
{
    use WithPagination;
    ...
    public function iniciarConferencia($orcamentoId)
    {
        $orcamento = Orcamento::findOrFail($orcamentoId);
        $orcamento->update(['workflow_status' => 'em_conferencia']);
        
        session()->flash('message', 'Conferência iniciada com sucesso!');
    }

    public function finalizarConferencia($orcamentoId)
    {
        $orcamento = Orcamento::findOrFail($orcamentoId);
        $orcamento->update(['workflow_status' => 'conferido']);
        
        session()->flash('message', 'Conferência finalizada com sucesso!');
    }
    ...
    public function render()
    {
        $orcamentos = Orcamento::query()
            ...
            // Filtrar orçamentos em conferência (incluindo finalizados)
            ->whereIn('workflow_status', ['aguardando_conferencia', 'em_conferencia', 'conferido', 'finalizado'])
        ...
        return view('livewire.lista-conferencia', [
